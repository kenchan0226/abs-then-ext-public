...
from data.batcher import coll_fn_control_gen, prepro_fn_control
from data.batcher import convert_batch_copy_control, batchify_fn_copy_control
...
class ControllableMatchDataset(CnnDmDataset):
    """ single article sentence -> single abstract sentence
    (dataset created by greedily matching ROUGE)
    """
    def __init__(self, split, filter_out_low_recall=False, n_compression_levels=2):
        super().__init__(split, DATA_DIR)
        self.filter_out_low_recall = filter_out_low_recall
        self.n_compression_level = n_compression_levels

    def _compute_level_from_ratio(self, ratio):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_compression_levels', type=int, action='store', default=2,
